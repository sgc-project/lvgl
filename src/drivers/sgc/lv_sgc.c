 ...
 * does. A device that APPEARS while the app runs reaches it through the
 * advertised list the daemon pushes: the driver takes the entries it has not
 * been offered before. A mouse also gets a CURSOR: LVGL draws none by itself,
 * and the driver is where the pointer binding belongs, so the arrow is drawn
 * here and taken away when the last mouse is.
 *
...
#if LV_SGC_INPUT && LV_USE_IMAGE
    /* The pointer cursor lives in the image widget; LVGL has no built-in one. */
...
typedef struct {
    ...
    bool releasing;        /**< True while this driver deletes its own indev */
} lv_sgc_input_t;
...
typedef struct {
    ...
    lv_obj_t * cursor;           /**< The pointer cursor, NULL when no mouse is held */
    bool cursor_dirty;           /**< A device went away: re-check the cursor on the next tick */
...
static void pump_cb(lv_timer_t * t)
{
    ...
    /* A device can go away without the daemon saying anything (LVGL deletes the
     * input device whose read failed); the cursor is re-checked here rather than
     * from inside that deletion. */
#if LV_SGC_INPUT
    if(ctx.cursor_dirty) {
    ...
    if(ev.kind == SGC_EVENT_ADVERTISED) {
        /* The daemon's resource list changed: a device was plugged in or
         * removed. The display is ours already; the devices are taken here, and
         * without that one that appears while this app runs never reaches it. */
#if LV_SGC_INPUT
        inputs_adopt();
#endif
        return;
...
static void disp_delete_cb(lv_event_t * e)
{
    ...
    /* The display (and the layers the cursor lives in) went with it. */
    ctx.cursor = NULL;
    release_controller();
...
#if LV_USE_IMAGE
/* LVGL 9 draws no cursor of its own: a pointing device without one is invisible
 * on screen. The classic 12x19 arrow - black outline over a white fill - lives
 * here because the driver owns the input binding and an application should not
 * have to hand-draw a pointer. ARGB8888, so the bytes are B, G, R, A. */
...
static const lv_image_dsc_t cursor_arrow_dsc = {
    .header.magic = LV_IMAGE_HEADER_MAGIC,
    .header.cf = LV_COLOR_FORMAT_ARGB8888,
    .header.w = 12,
    .header.h = 19,
    .header.stride = 12 * 4,
    .data_size = sizeof(cursor_arrow),
    .data = cursor_arrow,
};

/** Bind the cursor to every mouse held, and take it away when none is left. */
static void cursor_update(void)
{
    bool pointed = false;

    for(int i = 0; i < SGC_MAX_INPUTS; i++) {
        if(ctx.inputs[i].indev && ctx.inputs[i].resource.kind == SGC_RESOURCE_MOUSE) pointed = true;
    }

    if(ctx.disp == NULL) return;

    if(pointed && ctx.cursor == NULL) {
        ctx.cursor = lv_image_create(lv_display_get_screen_active(ctx.disp));
        lv_image_set_src(ctx.cursor, &cursor_arrow_dsc);
        LV_LOG_INFO("sgc: pointer cursor created");
    }
    else if(!pointed && ctx.cursor != NULL) {
        lv_obj_delete(ctx.cursor);
        ctx.cursor = NULL;
        LV_LOG_INFO("sgc: pointer cursor removed");
    }

    for(int i = 0; i < SGC_MAX_INPUTS; i++) {
        if(ctx.inputs[i].indev && ctx.inputs[i].resource.kind == SGC_RESOURCE_MOUSE) {
            lv_indev_set_cursor(ctx.inputs[i].indev, ctx.cursor);
        }
    }
}
#else
/** No image widget: there is nothing to draw a cursor with. */
static void cursor_update(void) { }
#endif

/**
 * LVGL's evdev driver deletes an input device by itself when the device under it
 * goes away (its read fails with ENODEV, see lv_evdev.c) - no revoke, no event
 * from the daemon. The pointer this driver keeps would then be stale, and the
 * next revoke or re-grant would free it a second time and take the application
 * down with it, so the slot is cleared here instead.
 */
static void input_deleted_cb(lv_event_t * e)
{
    lv_sgc_input_t * slot = lv_event_get_user_data(e);
    lv_indev_t * indev = lv_event_get_target(e);

    if(slot == NULL || slot->indev != indev) return;
    slot->indev = NULL;

    if(slot->releasing) return; /* our own delete, the caller logs it */

    ctx.cursor_dirty = true;
    LV_LOG_INFO("sgc: %s{%d} went away - its LVGL input device is gone, the claim is kept",
                kind_name(slot->resource.kind), slot->resource.index);
}

/** Delete an input device this driver owns, and keep the slot in step. */
static void input_delete(lv_sgc_input_t * slot)
{
    slot->releasing = true;
...
static void input_attach(sgc_resource r, int fd)
{
    ...
    lv_indev_add_event_cb(indev, input_deleted_cb, LV_EVENT_DELETE, slot);
    slot->indev = indev;
    LV_LOG_INFO("sgc: %s{%d} attached as an LVGL input device", kind_name(r.kind), r.index);

    /* A mouse with no cursor is a mouse nobody can see: draw one while one is
     * held (LVGL puts it in the display's system layer, above the UI). */
    cursor_update();
}
...
/** Is this device in the list the daemon last advertised to us? */
static bool input_is_wanted(sgc_resource r)
{
    for(size_t i = 0; i < ctx.wanted_count; i++) {
        if(ctx.wanted[i].kind == r.kind && ctx.wanted[i].index == r.index) return true;
    }
    return false;
}

/**
 * A device appeared while the app runs: the daemon pushed a changed list. Take
 * the devices this app has not been offered yet - that delta is what "new"
 * means, so a refusal is not asked for again on every push, while a device that
 * left the list and came back IS asked for again.
 */
static void inputs_adopt(void)
{
    sgc_resource * advertised = NULL;
    size_t count = 0;
    sgc_resource fresh[SGC_MAX_INPUTS];
    size_t fresh_count = 0;

    if(sgc_advertised(ctx.client, &advertised, &count) != 0) return;

    for(size_t i = 0; i < count && fresh_count < SGC_MAX_INPUTS; i++) {
        if(!is_input_kind(advertised[i].kind)) continue;
        if(input_is_wanted(advertised[i])) continue;
        fresh[fresh_count++] = advertised[i];
    }

    /* Remember before asking: the list is what makes a device "not new" next
     * time, so a refusal stays quiet until the device leaves the list. */
    inputs_remember(advertised, count);
    sgc_free(advertised);

    if(fresh_count == 0) return;

    LV_LOG_INFO("sgc: %d new input device(s) advertised - acquiring", (int)fresh_count);
    inputs_acquire(fresh, fresh_count);
}
